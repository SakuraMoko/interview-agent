  ...
  <div class="auth-page">
    <div class="auth-card">
      ...
      <form @submit.prevent="handleRegister">
        ...
        <div class="form-group">
          <label>密码</label>
          <input v-model="form.password" type="password" placeholder="至少8位，含字母+数字+特殊字符" :class="{ 'input-error': form.password && !passwordValid }" required />
          <ul v-if="form.password && passwordErrors.length" class="password-hints">
...
const passwordErrors = computed(() => {
  ...
  if (!pwd) return errs
  if (pwd.length < 8) errs.push('至少8个字符')
  if (!/[A-Za-z]/.test(pwd)) errs.push('至少包含一个字母')
  if (!/\d/.test(pwd)) errs.push('至少包含一个数字')
  if (!/[!@#$%^&*(),.?":{}|<>]/.test(pwd)) errs.push('至少包含一个特殊字符')
  return errs
})
const passwordValid = computed(() => form.password && passwordErrors.value.length === 0)
...
.password-hints { list-style: none; padding: 0; margin: 6px 0 0; font-size: 12px; color: #ef4444; text-align: left; }
